...
class Rocket {
  ...
  void fitness() {
    ...
    if (hitObstacle) fitness *= 0.01; // lose 90% of fitness hitting an obstacle
...
void setup() {
  ...
  obstacles.add(new Obstacle(0, 0, 1, height));
  obstacles.add(new Obstacle(width - 1, 0, 1, height));
  obstacles.add(new Obstacle(0, height - 1, width, 1));
  obstacles.add(new Obstacle(0, 0, width, 1));
}
...
void mouseDragged() {
  if (!dragging) {
    tempobstacle = new Obstacle(mouseX, mouseY, 0, 0);
  }
  dragging = true;
}
...
void mouseReleased() {
  if (dragging) {
    tempobstacle.w = mouseX - tempobstacle.position.x;
    tempobstacle.h = mouseY - tempobstacle.position.y;
