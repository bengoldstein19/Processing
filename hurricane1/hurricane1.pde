...
class Ball {
  ...
  void update() {
    ...
    if (location.mag() > 250) {
      location.normalize().mult(250);
      float velox = velocity.x;
      velocity.x = velocity.x*cos(2*angle) + sin(2*angle)*velocity.y;
      velocity.y = sin(2*angle)*velox - cos(2*angle)*velocity.y;
      velocity.mult(-1);
    }
...
void keyPressed() {
  if (key == 'r') {
     for (int i = 0; i < balls.length; i++) {
       ...
       balls[i].location.x = 80*cos(angle);
       balls[i].location.y = 80*sin(angle);
       balls[i].velocity.x = 5*cos(angle);
       balls[i].velocity.y = 5*sin(angle);
    }
  }
}